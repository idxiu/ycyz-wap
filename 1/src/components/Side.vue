<template>
    <div class="sidedrawer">
        ...
        <div class="content-mask" v-on:click="toggle" ref="contentMask"></div>
        <!--固定定位-->
        <div ref="content" class="position">
            <div @click="toggle" class="sideMenu iconfont icon-sy_menu"></div>
...
<script>
    import {once} from '../assets/js/dom';
    /**
     * 侧滑菜单容器
     *
    ...
    export default {
        ...
        mounted() {
            ...
            g.position = g.$refs.position; // 定位容器
        ...
        computed: {
            drawerStyle() {
                ...
                return {
                    ...
                    'z-index': swipeType === 'fixDrawer' ? '0' : '8',
                }
            },
            contentStyle() {
                let {swipeType} = this;
                return {
                    'z-index': swipeType === 'fixContent' ? '0' : '8'
                }
            }
        },
        watch: {
        ...
        methods: {
            ...
            onDrag(evt) {
                ...
                swiping = !(x < 5 || (x >= 5 && y >= x * 0.53));//x*num 的值越大越灵敏
                if (!swiping) return;
                evt.preventDefault();
                let offset;
                if (g.isDO) {
