<template>
	<div class="generalizeLink">
		<Header :title="'推广链接'" :showRight="true" :rooter="'-1'"></Header>
		<div class="content">
			...
			<!--添加推广链接弹框 -start -->
			<div v-show="isShowAddMask" v-if="renderdom">
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                size: this.HTML_FONT_SIZE * 1.33333,//列表二维码大小
        ...
        methods: {
            ...
            submitLink () {
                ...
                func.setSpreadAdd({data: postData}).then(() => {
                    this.isShowAddMask = false;
                    this.isSubmit = false;
                    this.getList();
                    this.postData=[];//重置是为了将填写的数据重新初始化
                    setTimeout(()=>{
                        this.$toast({ message: '链接生成成功', duration: 2000});
					},2000)
        ...
        watch: {
            ...
            isShowDetailMask (newVal, oldVal) {
                if (newVal) {
                    this.ModalHelper.open();
                } else {
                    this.ModalHelper.close();
...
<style scoped lang="less">
	...
	.content {
		padding: 1.22667rem /* 92/75 */ 0.40rem 0.40rem;
		.generalizeLink-add {
			margin-top: 0.40rem;
			height: 1.07rem;
			line-height: 1.07rem;
