        ...
        <div class="hasmenu">
            ...
            <mt-tab-container v-model="indexx">
                        ...
                        <div class="content">
                            <div class="pk-input pk-inputpro">
                                ...
                                <div class="input fs-12 input_pro" style="border-bottom: 1px solid #fff;">
                                    <input name="oldPwd" autocomplete="off" v-model="oldLoginPwd" v-validate="'required'" type="password" placeholder="请输入您的原登录密码">
                ...
                <mt-tab-container-item id="2">
                    ...
                    <div class="contenerf">
                        <div class="content">
                            <div v-if="checkdrawpass" class="pk-input pk-inputpro">
                                <label class="title muster fs-14">原取款密码</label>
                                <div class="input fs-12 input_pro" style="border-bottom: 1px solid #fff;">
                                    <input name="oldoutPwd" autocomplete="off" v-model="oldTake" v-validate="'required'" :class="{'input': true, 'is-danger': errors.has('oldoutPwd') }" type="password" placeholder="请输入您的原取款密码">
                                    <i type="clear" v-show="errors.has('oldoutPwd')" class="fs-15 iconfont icon-czsb tj"></i>
                                </div>
                            </div>

                            <div v-else-if="!checkdrawpass" class="pk-input pk-inputpro">
                            ...
                            <div class="pk-input pk-inputpro">
                                ...
                                <div class="input fs-12 input_pro">
                                    <input maxlength="6" type="password" name="outPwd" autocomplete="off" v-model="newTake" v-validate="'required|numeric|max:6|min:6'" :class="{'input': true, 'is-danger': errors.has('outPwd') }" placeholder="请输入您的取款密码">
                            ...
                            <div class="pk-input pk-inputpro">
                                <label class="title muster fs-14">确认密码</label>
                                <div class="input fs-12 input_pro" style="border-bottom: 1px solid transparent!important;">
                                    <input maxlength="6" name="confirmOutPwd" class="no-bod-bot" autocomplete="off" v-model="reTake" v-validate="{'required': 'true', 'is': newTake}" :class="{'input': true, 'is-danger': errors.has('confirmOutPwd') }" type="password" placeholder="请再次输入您的取款密码">
                                    <i type="clear" v-show="errors.has('confirmOutPwd')" class="fs-15 iconfont icon-czsb tj"></i>
                                </div>
                            </div>
                        </div>
                        <div class="else">
                            <div v-if="checkdrawpass" class="divs">
                                <p v-show="errors.has('loginPwd')" class="errs help fds-12 is-danger">原取款密码不能为空！</p>
                            ...
                            <div v-else-if="!checkdrawpass" class="divs">
...
<script>
    ...
    export default {
        ...
        methods: {
            getquery(){
                withdrawQuery().then(res => {
                    if(res.isSet){
                        this.checkdrawpass = true
                    }else{
                        this.checkdrawpass = false
                    }
                }).catch(err => {
                    this.$toast({
                        message: err,
