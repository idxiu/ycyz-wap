<template>
	<div class="spread">
		...
		<div class="spread-title">
			<h1>我的推广链接</h1>
			<router-link tag="div" :to="{name:'backCommissionRatio'}" class="right">查看返佣比例</router-link>
		</div>
		<div class="spread-link">
			<input class="link" v-model="link" readonly>
			<button type="button" v-clipboard:copy="link" v-clipboard:success="onCopy" v-clipboard:error="onError">
				复制链接
			</button>
		</div>
		<div class="spread-title">
	...
	.spread {
		...
		.spread-title {
			...
			.right {
				...
				line-height: 0.4rem;
				color: @color-x;
				border-bottom: 1px solid @color-x;
			}
		}
		.spread-link {
			padding: 0 0.4rem;
