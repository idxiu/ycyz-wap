        ...
        <div v-if="list.length != 0" class="order-list">
            ...
            <div class="page-loadmore-wrapper" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
                    <pk-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :top-all-loaded="topAllLoaded" @top-status-change="handleTopChange" @bottom-status-change="handleBottomChange" ref="loadmore" :stop-translate="stopTranslate">
                        <ul>
                <li class="pk-1px-b" @click="toggle(tvgames)" v-for="tvgames in list" :key="tvgames.id">
                    <div class="top clearfix">
                        <div class="text-dots left">{{tvgames.gameTranslatedName}}</div>
                        <div class="text-dots right winlose">{{tvgames.statusName?tvgames.statusName:'未结算'}}</div>
                        <div class="text-dots right">{{tvgames.betAll}}</div>
                        <div class="text-dots right">{{tvgames.win}}</div>
                    </div>
                    <div class="center clearfix">
                        <div class="left">{{tvgames.productName}}</div>
                    </div>
                    <div class="bottom clearfix">
                        <div class="text-dots left">注单号：{{tvgames.orderId}}</div>
                        <div class="text-dots right">{{tvgames.betTime|filterDate}}</div>
                        <div class="text-dots right">{{tvgames.periodsOrTable}}期</div>
                    </div>
                    <div class="noteShow clearfix" v-show="tvgames.show">
                        <!-- <div class="details clearfix">
                                    <div class="details-tit">场景编号：</div>
                                    <div class="details-cont">{{tvgames}}</div>
                                </div> -->
                        <div class="details clearfix">
                            <div class="details-tit">游戏明细：</div>
                            <div class="details-cont">{{tvgames.betDetail}}</div>
                        </div>
                    </div>
                    <div class="arrowIcon iconfont icon-order-moreinfo fs-10" v-bind:class="{'up':tvgames.show,'down':!tvgames.show}"></div>
                </li>
            </ul>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList(t) {
               lotteryList(1, this.chooseTimeVal.key, this.chooseMainVal.value, this.page, this.pageSize, '')
                    .then(res => {
            ...
            getKindDrop() {
                let kindData = new wallet.TopIdRep();
                kindData.setId(this.chooseMainVal.topid);
                // console.log(this.chooseMainVal.topid);
                const data = kindData.serializeBinary();
                this.ajax('REPORT_KIND_DROP', wallet.LotteryTicketTypeResp, data).then(res => {
                    res.lotterytickettypeList.unshift({
                        fcHref: "",
                        fcname: "全部彩种",
                        id: 0,
                    })
                    this.mainKindType[0].values = res.lotterytickettypeList;
                }).catch(err => {
