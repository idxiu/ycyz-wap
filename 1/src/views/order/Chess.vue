        ...
        <div v-if="list.length != 0" class="order-list">
             <div class="page-loadmore-wrapper" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
                    <pk-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :top-all-loaded="topAllLoaded" @top-status-change="handleTopChange" @bottom-status-change="handleBottomChange" ref="loadmore" :stop-translate="stopTranslate">
                        <ul>
                            <li class="pk-1px-b" @click="toggle(chessLists)" v-for="chessLists in list" :key="chessLists.id">
                                <div class="top clearfix">
                                    <div class="text-dots left">{{chessLists.gameTranslatedName}}</div>
                                    <div class="text-dots right winlose">{{chessLists.statusName?chessLists.statusName:'未结算' }}</div>
                                    <div class="text-dots right">{{chessLists.betAll}}</div>
                                    <div class="text-dots right">{{chessLists.win}}</div>
                                </div>
                                <div class="center clearfix">
                                    <div class="left">{{chessLists.productName}}</div>
                                </div>
                                <div class="bottom clearfix">
                                    <div class="text-dots left">注单号：{{chessLists.orderId}}</div>
                                    <div class="text-dots right">{{chessLists.betTime|filterDate}}</div>
                                    <div class="text-dots right">{{chessLists.periodsOrTable}}期</div>
                                </div>
                                <div class="noteShow clearfix" v-show="chessLists.show">
                                    <!-- <div class="details clearfix">
                                                                    <div class="details-tit">房间号：</div>
                                                                    <div class="details-cont">{{chessLists}}</div>
                                                                </div> -->
                                    <div class="details clearfix">
                                        <div class="details-tit">游戏明细：</div>
                                        <div class="details-cont">{{chessLists.betDetail}}</div>
                                    </div>
                                </div>
                                <div class="arrowIcon iconfont icon-order-moreinfo fs-10" v-bind:class="{'up':chessLists.show,'down':!chessLists.show}"></div>
                            </li>
                        </ul>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList(t) {
                lotteryList(8, this.chooseTimeVal.key, this.chooseMainVal.value, this.page, this.pageSize, '')
                    .then(res => {
                        console.log(res, "res");
            ...
            toggle(chessLists) {
                chessLists.show = !chessLists.show;
            },
            //选择器
