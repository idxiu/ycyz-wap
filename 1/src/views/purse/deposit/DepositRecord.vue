<template>
	<div class="record">
		...
		<!-- 选择时间 -->
		<mt-popup :closeOnClickModal="true" :position="'bottom'" v-model="popupVisibleTime"
				  style="width: 100%;z-index: 2003;">
			<div class="popup-title pk-1px-b">
				<span @click="cancel()">取消</span>
				<span @click="sure()">确定</span>
			</div>
			<mt-picker valueKey="value" :itemHeight="itemHeight" :slots="slotsTime"
					   @change="onValuesChangeTime"></mt-picker>
		</mt-popup>
			...
			<div class="page-loadmore">
				<div class="page-loadmore-wrapper" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
					<pk-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded"
						...
						<ul>
							<!-- 公司存款 -->
							<li @click="handleModalShow(item)" v-if="chooseTypeShow.value===1"
								:class="{'pk-1px-b':index!=list.length-1}" v-for="(item,index) in list" :key="index">
								<h2>
									<span>{{item.bankName}}</span>
									<span>{{item.depositMoney}}(优惠{{item.depositDiscount+item.otherDiscount}})</span>
							...
							<!-- 线上存款 -->
							<li @click="handleModalShow(item)" v-if="chooseTypeShow.value===2"
								:class="{'pk-1px-b':index!=list.length-1}" v-for="(item,index) in list" :key="index">
								<h2>
									<span>{{item.depositInstitution}}</span>
									<span>{{item.amountDeposit}}(优惠{{item.otherDepositDiscount+item.depositDiscount}})</span>
									<span class="fail" :class="{'inHand':item.status=== 1,'success':item.status=== 2}">
                                        <!-- 1处理中 2成功 3失败 -->
                                        {{item.status===1 ?'处理中':item.status===2?'成功':'失败'}}
                                    </span>
							...
							<!-- 取款 -->
							<li @click="handleModalShow(item)" v-if="chooseTypeShow.value===3"
								:class="{'pk-1px-b':index!=list.length-1}" v-for="(item,index) in list" :key="index">
								<h2>
									<span>{{item.outwardMoney}}(费用{{item.feeMoney}})</span>
									<span class="fail"
										  :class="{'inHand':item.status=== 1 || item.status=== 3 || item.status=== 6,'success':item.status=== 2}">
							...
							<!-- 额度转换 -->
							<li v-if="chooseTypeShow.value===4" :class="{'pk-1px-b':index!=list.length-1}"
								...
								<h2>
									...
									<!--1处理中 2失败 3成功-->
									<span class="success" :class="{'inHand':item.status=== 1,'fail':item.status=== 2}">{{item.status === 1?'处理中':item.status === 2?'失败':'成功'}}</span>
								</h2>
								<p>
									<span>订单号：{{item.orderId}}</span>
									<span>{{item.createTime | filterDate('MM-DD HH:mm')}}</span>
        ...
        methods: {
            ...
            getList (t) {
                let postData = {
                    ...
                    timeType: this.chooseTimeShow.key,
                }
