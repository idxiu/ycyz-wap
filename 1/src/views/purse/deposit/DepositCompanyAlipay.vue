			...
			<div class="deposit-info">
				...
				<ul>
					<li class="pk-1px-b">
						...
						<input v-if="baseInfoData.isInteger===2" name="money" type="tel" v-model="postData.depositMoney"
							   v-validate="`required|between:${baseInfoData.lineDepositMin},${baseInfoData.lineDepositMax}`"
							   placeholder="请输入存款金额" :class="{'input': true, 'is-danger': errors.has('money') }">
						<input v-if="baseInfoData.isInteger===1" name="money" type="tel" v-model="postData.depositMoney"
							   v-validate="`numeric|required|between:${baseInfoData.lineDepositMin},${baseInfoData.lineDepositMax}`"
							   placeholder="请输入存款金额" :class="{'input': true, 'is-danger': errors.has('money') }">
						<i @click="postData.depositMoney=''" v-show="errors.has('money')"
						   class="fs-16 iconfont icon-login-error icon-style  error-icon"></i>
					</li>
					<div v-show="postData.depositMoney" class="money-chinese">{{depositMoneyChinese}}</div>

					<li class="pk-1px-b">
						<span class="must">存款人姓名</span>
						<input name="inAccount" type="text" v-model="postData.depositAccount" v-validate="'required'"
							   placeholder="请输入存款账号" :class="{'input': true, 'is-danger': errors.has('inAccount')}">
						<i @click="postData.depositAccount=''" v-show="errors.has('inAccount')"
						   class="fs-16 iconfont icon-login-error icon-style  error-icon"></i>
					</li>
					<li class="pk-1px-b">
						<span class="must">存款时间</span>
						<input name="inTime" @click="openPicker()" type="text" v-model="postData.depositTime"
							   v-validate="'required'" readonly placeholder="请选择时间">
						<i class="iconfont icon-qb-time"></i>
						<i @click="postData.depositTime=''" v-show="errors.has('inTime')"
						   class="fs-16 iconfont icon-login-error icon-style  error-icon"></i>
					</li>
