                        ...
                        <div class="link-list">
                            <swipe-list ref="list" class="card" :disabled="!enabled" :items="list" item-key="id" @swipeout:click="itemClick()" @rightRevealed="slideRight($event)">
                                <template v-slot="{ item, index, close }">
                                            <div class="box-parent" :class="{'box-unRead':!item.read}" @click="goPath(item.id, 2)">
                                                <h4 class="box-title">
                                                    <div><span class="red-dot" v-if="!item.read"></span> {{item.advisoryTitle}}</div>
                                                    <div>{{item.advisoryTime | filterDate}}</div>
                                                </h4>
                                                <div class="box-cent" >
                                                    {{ item.advisoryContent }}
                                                </div>
                                            </div>
</template>
        ...
        methods: {
            getList() {
                ...
                getInboxMyList({
                    ...
                    if (res.success) {
                        if (res.data) {
                            let result = res.data.data ? res.data.data.dataList : [];
            ...
            readAll() {
                ...
                getInboxMyStatus(data).then(res => {
                    if (res.success) {
