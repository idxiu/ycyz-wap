...
//获取群投注计划
function getPlanneds(data) {
    return axios.post(api.GETPLANNED_INFOS, data).then(res => {
        return res.data;
    });
}
...
//跟投提交
function postPlanned(data) {
    return axios.post(api.POST_PLANNED_INFO, data).then(res => {
        return res.data;
    });
}
...
//获取通知列表
function getNotices() {
    return axios.post(api.GET_NOTICES).then(res => {
        return res.data;
    });
}
