 ...
 * @LastEditTime: 2019-10-12 09:55:26
    ...
    <div>
        ...
        <div class="pk-chat-info">
            ...
            <div class="pk-chat-member" v-show="memberList.length > 0">
                <ul>
                    <li v-for="(item,index) in memberList" :key="index" @click="toUserInfo(item)">
                        <img src="../../assets/img/game-icon/yxdt_lts.png" alt
                            v-if="item.adminWordsPerm == 1 || item.adminWordsPerm == 3" class="pk-chat-icon" />
                        ...
                        <div class="pk-member-online" v-if="item.is_online"
                            :class="item.is_online == 1 ?'pk-online':''"></div>
                        <div>
                ...
                <div class="pk-member-more" v-if="allMemberLen.length > 15" @click="isShowMore = !isShowMore">
            ...
            <div class="pk-chat-btns" v-if="isExisted == 1">
                <div class="pk-chat-jion" @click.stop="InChat()">进入群聊</div>
            </div>
...
export default {
    ...
    methods: {
        ...
        InChat() {
            let gameInfo = {
                gameType: this.info.gameType,
                platform: this.info.platformName,
                gameId: this.info.gameId,
                isWh: this.info.isWh
            };
            sessionStorage.setItem(
                "gameInfo",
                encodeURIComponent(JSON.stringify(gameInfo))
            );
            this.$router.replace({
                name: "chat",
                query: {
                    id: this.info.id,
                    roomType: this.info.groupType,
                    name: this.info.name
                }
            });
        },
