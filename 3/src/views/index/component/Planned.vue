<template>
    <div id="planned">
        <div class="planned" :class="{'goDetail':isDetail}" v-show="planShow">
            <swiper :options="swiperOption" ref="mySwiper" :class="{'unHavePagination':swiperSlides.length <= 1,'havePagination':swiperSlides.length > 1}">
                <swiper-slide @click.native="goDetail(item)" class="plan-swiper" v-for="(item, index) in swiperSlides" :key="index">
                    <div v-if="!isDetail" class="betPlan-headImg">
                    ...
                    <div @click.stop="followBet(item)" class="betPlan-follow" :class="{'unTouch': item.listIndex.start == 2 || item.listIndex.start == 3}">跟投</div>
                    <div v-if="!isDetail" class="face-item iconfont iconicon_info_gtplan_close" @click.stop="deleteFace()" :class="{'close':item.length<20}">
                        <!-- <img :src="closeIcon" /> -->
                    </div>
                </swiper-slide>
        ...
        <div class="min-planned iconfont iconicon_info_gtplan_entrance" v-show="!planShow" @click="showPlanned()">
            计划
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        followBet(item){
            if(item.listIndex.start == 1){
                this.$emit('show-plan-modal',item)
            }
        },
        countdown(){
                ...
                if(this.$route.query.roomType == MSG_TYPE.TYPE_PUBLIC || this.$route.query.roomType == MSG_TYPE.TYPE_PRIVATE){
                    for(var i= 0; i < this.messageList.length;i++){
                        ...
                        for(var j=0;j < newList.length;j++){
                            ...
                            adminInfo.push({
                                isAdmin : this.messageList[i].isAdmin,
                                mid : this.messageList[i].senderId,
                                senderName : this.messageList[i].senderName
                            })
                        }
                    }
                }else{
                    relArray.push(this.messageList[0].message);
                    adminInfo.push({
                        isAdmin : this.messageList[0].isAdmin,
                        mid : this.messageList[0].senderId,
                        senderName : this.messageList[0].senderName
                    })
                }
            }
            relArray.map((item,index)=>{
                ...
                item.contentId = item.betPlanSub[listIndex.index].contentId; 
                item.listIndex = listIndex;
                //发送者信息
                item.isAdmin = adminInfo[index].isAdmin;
                item.mid = adminInfo[index].mid;
                item.senderName = adminInfo[index].senderName;
                item.senderHead = item.betPlanSub[listIndex.index].plannerPhoto;
                item.msg = {
                    noticeType:MSG_TYPE.IM_NOTICE_FOLLOW,
                }
            ...
            this.$emit('insert-plan',myNewData)
        },
        goDetail(item){
            if(!this.isDetail){
                this.$router.push({
                    ...
                    query:{
                        pid: item.pid,
                        room: this.planmessage[0].roomId,
            ...
            this.swiperSlides.map((item,index)=>{
                ...
                if (rightTime > 0) {
                    ...
                    if(item.listIndex.start == 1){
                        ...
                        this.$emit('insert-plan',[this.swiperSlides[index]])
                    }
                }
            })
    ...
    .swiper-container{
        .plan-swiper{
            ...
            .face-item{
                ...
                background-color: $inputbotline-color;
                box-shadow: 0 0 px2rem(10) 0
		        rgba(0, 0, 0, 0.2);
            }
        }
