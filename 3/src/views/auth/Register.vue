	...
	<div class="pk-auth">
		<div class="pk-auth-box">
				...
				<ul>
					...
					<li class="form-tool">
						<div class="tool-item checkbox">
							<input type="checkbox" name id="rember" v-model="agree" />
							<label for="rember">同意</label>
							<span class="agreement" @click="agreementText = true">《注册协议》</span>
						</div>
					</li>
				</ul>
				<div class="btns">
					<div class="btn btn-register" @click="toRegister">注册</div>
				</div>
				<div class="pk-register-in">
					已有账号，
					<router-link tag="a" :to="{name:'login'}" class="tool-item kf">直接登录</router-link>
				</div>
				<!-- 验证码 start -->
			...
			<nut-dialog
				...
				:visible="agreementText"
				:lockBgScroll="false"
				:closeOnClickModal="false"
				:noCancelBtn="true"
				:noFooter="true"
				@close="agreementText=false"
			>
				<div class="pk-agreement">
					<p>尊敬的客户，欢迎您注册成为本网站用户，在注册前请仔细阅读如下服务条款：</p>
					<p>
					...
					<p>1.3本协议不涉及您与本网站的其他用户之间因网上交易而产生的法律关系及法律纠纷。但您在此不可撤销地同意，您将全面接受并履行与本网站其他用户在本网站签订的任何电子法律文本，并承诺按照该法律文本享有和（或）放弃相应的权利、承担和（或）豁免相应的义务。</p>
				</div>

				<div class="pk-agreement-bnt" @click="agree=true,agreementText=false">确认并继续</div>
			</nut-dialog>
			<!--注册协议 -end -->
		</div>
	</div>
...
import { getAnnouncement } from "../../services/index";
